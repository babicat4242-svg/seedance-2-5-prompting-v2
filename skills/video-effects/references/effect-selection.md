...
확인한 시각 변화와 새로 제안하는 변주를 구분하고, 시작·발동·변형·카메라·끝·연결을 채운다. 이름만 아는 효과를 공식 동작처럼 발명하지 않는다. 예를 들어 Lidar transition의 공개 설명이 Incline과 같아 의미가 불명확하면, 그 텍스트만으로 점군 스캔 효과라고 단정하지 않는다.
